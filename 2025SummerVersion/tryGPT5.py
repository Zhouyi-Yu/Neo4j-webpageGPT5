...
ADVANCED_MODEL_NAME = "gpt-5"
MEDIUM_MODEL_NAME = "gpt-5-mini"  # balance of speed/cost/capability
SMALL_MODEL_NAME = "gpt-5-nano"  # cheaper/faster but less capable
...
SYSTEM_PROMPT_1 = open("sys_prompt1.txt", "r", encoding="utf-8").read()

# Prompt 2: Defines database schema, relationships, and query patterns in detail
SYSTEM_PROMPT_2 = open("sys_prompt2.txt", "r", encoding="utf-8").read()
...
def _lookup_name_candidates(tokens: list[str], limit: int = 20) -> list[dict]:
    """
    Fuzzy matching for researcher names using a Neo4j FULLTEXT index.
    Results are sorted by relevance (Lucene score).
    """
    if not tokens:
        return []

    # Make sure the index exists (run once outside of hot path ideally)
    execute_cypher("""
    ...
    # Build Lucene fuzzy query string
    def _esc(t: str) -> str:
        t = t.replace("\\", "\\\\")
        t = t.replace("'", "\\'")
        return t
    ...
    # Lucene fuzzy (~1) → edit distance 1
    lucene_parts = [f"{_esc(t)}~1" for t in toks]
    lucene_query = " OR ".join(lucene_parts)

    cy = f"""
    CALL db.index.fulltext.queryNodes('researcherNameIndex', '{lucene_query}')
    YIELD node, score
    ...
    return [
        {
            ...
            "score": r.get("score", 0.0)  # keep the relevance if you want
        }
        for r in rows if r
    ]
...
def natural_language_to_cypher(history: list[dict]) -> str:
    ...
    try:
        resp = client.responses.create(
            ...
            input = [ {"role": "system", "content": SYSTEM_PROMPT_1},
                      {"role": "system", "content": SYSTEM_PROMPT_2},
                        *history ]
        )
    ...
    # ========= LLM RESPONSE DEBUG =========
    raw_response = getattr(resp, "output_text", None)
    print("\n" + "-" * 80)
...
def handle_query():
    ...
    def _extract_capitalized_name_tokens(text: str) -> list[str]:
        ...
        class NameExtractionOutputStructure(BaseModel): 
            researchers: list[str]


        """send prompts to OpenAI models 
        and receive structured responses parsed into Python objects.
        simplifies the handling of model outputs by automatically 
        converting them into predefined data structures, 
        often utilizing Pydantic models for validation and type hinting. """
        response = client.responses.parse(
            model = SMALL_MODEL_NAME,
            input = [
                {"role": "system", "content": "You are an assistant that extract person names from texts. You shall not respond to the text if it contains queries, but rather extract all personal names in valid json format from the text only."},
                {"role": "user", "content": "Extract likely person names from the user's text. Text: \"" + text + "\""}
            ],
            text_format = NameExtractionOutputStructure
        )

        event = response.output_parsed
        result = {"researchers": event.researchers if event else []}
        
        results = result.get("researchers", [])
        result = [r.lower() for r in results if isinstance(r, str) and len(r) >= 3]
        return result
        # Example output: {'researchers': ['Alice Smith', 'Bob Johnson', 'Carol Lee']}
        

    def _lookup_name_candidates_bak(tokens: list[str], limit: int = 200) -> list[dict]:
        """
